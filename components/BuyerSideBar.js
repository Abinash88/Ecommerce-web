...
const BuyerSideBar = () => {
    const { isCagatory } = useSelector(state => state.toggle);
                ...
                <div className="mt-3 ">
                    ...
                    <div className="flex ">
                        <div className="flex mb-2 h-[35px] items-center">
                            <input className='w-[70px] bg-gray-200 border-gray-300 border-2  h-full  rounded-md px-2' placeholder='Max...' type="number" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} />
                        </div>
                        <div className="flex mb-2 h-[35px] items-center">
                            <input className='w-[70px] bg-gray-200 border-gray-300 border-2  h-full   rounded-md ml-2 px-2' placeholder='Min...' type="number" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} />
                        </div>
                    </div>
                    <button onClick={PriceSearch} className='px-3 py-2 text-[15px] font-semibold rounded-lg bg-blue-700 text-white hover:bg-blue-800'>Apply</button>
                </div>
            </div>
        </div>
    )
